...
class AI:
    ...
    try:
        cache = open(file='../data/cache.p', mode='rb')
    except IOError:
        cache = open(file='../data/cache.p', mode='wb')
    ...
    def ai_move(self):
        ...
        if self.opening_moves:
            chosen_move = chess.Move.from_uci(self.opening_moves[randint(0, len(self.opening_moves) // 2)])
        else:
            for move in self.board.legal_moves:
                self.board.push(move)

                local_score = self.minimax(self.depth - 1, not self.is_ai_white, -1e8, 1e8)
                self.board_caches[self.hash_board(self.depth - 1, not self.is_ai_white)] = local_score

                if self.is_ai_white and local_score > global_score:  # MAX
                    global_score = local_score
                    chosen_move = move
                elif not self.is_ai_white and local_score < global_score:  # MIN
                    global_score = local_score
                    chosen_move = move
    ...
    def minimax(self, depth, is_maxing_white, alpha, beta):
        ...
        if depth == 0 or not self.board.legal_moves:
            self.board_caches[self.hash_board(depth, is_maxing_white)] = evaluate_board(self.board)
            return self.board_caches[self.hash_board(depth, is_maxing_white)]
        # else
        ...
        for move in self.board.legal_moves:
            ...
            self.board_caches[self.hash_board(depth - 1, not is_maxing_white)] = local_score
