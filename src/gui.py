    ...
    def move(self, dest_square):
        ...
        if move in legal_moves:
            ...
            if self.board.is_checkmate():
                ...
                self.label_status['text'] = "Computer's turn. The computer is thinking..."
            ...
            for col in range(COLUMN_NUMBER):
                ...
                if (row, col) in self.highlighted_pieces:
                    self.canvas.create_rectangle(
                        ...
                        fill=YELLOW,
                        tags='square')
                elif ((row, col) == self.start_square) and (self.selected_piece is not None):
                    self.canvas.create_rectangle(
                        start_column,
                        start_row,
                        end_column,
    ...
    def draw_pieces(self):
        ...
        for square in chess.SQUARES:
            ...
            if piece is not None:
                match piece.symbol():
                    case 'Q':
                        image_name = 'queen_white'
                    case 'q':
                        image_name = 'queen_black'
                    case 'B':
                        image_name = 'bishop_white'
                    case 'b':
                        image_name = 'bishop_black'
                    case 'K':
                        image_name = 'king_white'
                    case 'k':
                        image_name = 'king_black'
                    case 'N':
                        image_name = 'knight_white'
                    case 'n':
                        image_name = 'knight_black'
                    case 'P':
                        image_name = 'pawn_white'
                    case 'p':
                        image_name = 'pawn_black'
                    case 'R':
                        image_name = 'rook_white'
                    case 'r':
                        image_name = 'rook_black'

                image_path = '../img/%s.png' % (image_name)
                piece_name = '%s%s' % (piece.symbol(), square)

                if image_path not in self.icons:
                    image = Image.open(image_path).resize((64, 64))
                    self.icons[image_path] = ImageTk.PhotoImage(image)

                row = square // 8
                column = square % 8

                self.add_piece(piece_name, self.icons[image_path], row, column)
